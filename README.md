...
Runner defaults live in `chess-harness.config.json`. Use that file for normal defaults such as Codex model, effort, FastChess game count, time control, viewer port, analysis settings, learner autolearn, and preflight behavior. CLI arguments still work as one-off overrides for a run.
